...
** EDIT: ** Lisätäänkö muuttujat tässä, vai Data-luvussa? Lisätään tässä. Koitetaan aina pitää alkuperäinen data mahdollisimman "lähellä", luodaan siis kaikki uudestaan. Tarketeena .data jos koko aineisto ja .dat jos rajattu.
...
```{r G1_4_ga}
# ei kovin siisti - alla siistimpi, jossa luodaan ga2
...
ISSP2012esim1.dat <- mutate(ISSP2012esim1.dat, ga2 = case_when((age_cat == "1")&(sp == "m") ~ "m1",
                                       (age_cat == "2")&(sp == "m") ~ "m2",
                                       (age_cat == "3")&(sp == "m") ~ "m3",
                                       (age_cat == "4")&(sp == "m") ~ "m4",
                                       (age_cat == "5")&(sp == "m") ~ "m5",
...
ISSP2012esim1.dat %>% tableX(ga,ga2) # toimii oikein!
...
taulu46 <- ISSP2012esim1.dat %>% tableX(maa,ga2,type = "count")
kable(taulu46,digits = 2, caption = "Ikäluokka ja sukupuoli ga2")

taulu47 <- ISSP2012esim1.dat %>% tableX(maa,ga2,type = "cell_perc")
...
```{r ga1CA1}
gaTestCA1 <- ca(~ga2 + V6,ISSP2012esim1.dat)
...
ISSP2012esim1.dat <- mutate(ISSP2012esim1.dat, maaga = paste(maa, ga2, sep = ""))

ISSP2012esim1.dat %>% tableX(maa, maaga) # taitaa toimia?

str(ISSP2012esim1.dat$maaga)
...
**TARKISTA**
```{r maagaCA1}
maagaTestCA1 <- ca(~maaga + V6,ISSP2012esim1.dat)
par("cex"= 0.7)
